...
int main(int argc, char *argv[]) {
  ...
  if (!go_to_consolidate) {
    ...
    if (adaptive) {
      ...
      while (!stoppingCriteriaMet) {
        ...
        subnets.push_back(ARACNe3_subnet(
            subnet_matrix, i + 1, prune_alpha, method, alpha, prune_MaxEnt,
            output_dir, subnets_dir, log_dir, nthreads));
      ...
      if (nthreads > 1 && num_subnets > 1)
        std::cout
            << "Note: Because more than one thread is used to compute a fixed "
               "number of subnetworks (--adaptive not specified), completion "
               "times may not be in order.  This is not an error.\n"
            << std::endl;
      subnets = std::vector<reg_web>(num_subnets);
      for (int i = 0; i < num_subnets; ++i) {
        genemap subnet_matrix = sampleFromGlobalGenemap();
        subnets[i] = ARACNe3_subnet(subnet_matrix, i + 1, prune_alpha, method,
                                    alpha, prune_MaxEnt, output_dir,
                                    subnets_dir, log_dir, nthreads);
        ;
      }
    }
  ...
  if (!do_not_consolidate) {
    ...
    std::vector<consolidated_df_row> final_df =
        consolidate_subnets_vec(subnets);
